...
手机端发布为一个公开 HTTPS 网址。用户用手机 Safari 打开网址，登录账号，然后添加到主屏幕。之后手机和电脑不需要在同一个 Wi-Fi 或热点下，手机记账先进入云端草稿箱，电脑端登录同一账号后拉取并确认入库。
...
4. 以后从桌面图标打开即可记账。
5. 电脑端打开钱迹 WorthTrace，登录同一个账号，进入账号同步，刷新云端草稿并确认入库。
