...
if [ ! -d "$MOBILE_DIR" ]; then
  ...
  echo "请确认已经完整解压 worthtrace-mobile-pwa-0.3.32.zip。"
  read -r "?按回车退出。"
  exit 1
fi
...
URL="http://$IP:$PORT/index.html?mobileVersion=0.3.32&resetCache=1&syncUrl=http://$IP:$SYNC_PORT"
RESET_URL="http://$IP:$PORT/index.html?mobileVersion=0.3.32&resetCache=1&resetBinding=1&syncUrl=http://$IP:$SYNC_PORT"
...
echo ""
echo "钱迹 WorthTrace 手机端 PWA 0.3.32"
echo ""
echo "请把这个链接发到手机："
echo "$URL"
echo ""
