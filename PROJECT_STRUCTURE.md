...
手机端工作区。`mobile/pwa/` 是手机 PWA 正式源码（记账、花费、投资、财务健康四个模块），`mobile/mockups/` 是设计方向稿（不接正式数据）。手机端通过局域网配对或 Supabase 云端草稿与桌面端联动；「投资」模块记录买入/卖出，月底更新确认后写入 `investment_cashflows`（`source_kind = mobile_investment`），同日定投自动买入会被手机手动买入替代。
